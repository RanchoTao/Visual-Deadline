...
Before PR G, auth supported email/password signup, sign-in, email verification callback/resend, token refresh, sign-out, and local guest mode. PR G adds independently gated Google, GitHub, X/Twitter, and phone entry points; identity linking and production guest import remain disabled until their separate gates are approved.
