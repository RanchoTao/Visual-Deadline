...
Final reconciliation reads the actual persisted canonical rows. It compares the source mapping and target UUIDs, owner, canonical type, all PR-F source-backed fields and timestamps, valid relationships, provenance, mapping cardinality, and actual target checksum. A non-null stored FK that cannot resolve through the expected legacy mapping is a hard reconciliation failure, never normalized to `NULL`. A ledger record is marked `migrated` only after that comparison succeeds; unresolved, missing, self-referential, one-sided, conflicting, and cyclic relationships remain explicit evidence and are not silently repaired.
