...
- 本地持久化：使用 `localStorage` 保存项目、主观压力，并兼容旧版本任务数据。

## 认知压力层

- 当前时间会显示在优先级地图标题区，并每分钟刷新。
- “当前主观压力”滑杆会保存到 `localStorage`。
- 实时压力指数由主观压力、进行中项目的紧急度/重要性、完成项目和恢复/娱乐活动共同估算。
- 进行中项目显示在地图上；已完成和已放弃项目进入“生命日志 / Activity Log”。
- 旧数据会自动补齐 `activityType`、`lifecycleStatus`、归档时间等字段。
