...
export function AuthPanel(props: AuthPanelProps) {
  const { isConfigured, isLoading, error, status: authStatus, authDebugInfo, onSignIn, onSignUp, onResendVerification, onVerifyEmailOtp, featureFlags, onOAuth, onRequestPhoneOtp, onVerifyPhoneOtp, phoneResendRemainingMs, emailResendRemainingMs } = props;
  ...
  return (
    <main className="min-h-screen bg-[#fbfbfa] px-4 py-10 text-zinc-900 sm:py-16">
      <section className="mx-auto max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-[0_18px_55px_-45px_rgba(0,0,0,.45)] sm:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-zinc-400">VISUAL DEADLINE</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-zinc-950">登录到 VD</h1>
        <p className="mt-3 text-sm leading-6 text-zinc-500">使用你的账号进入个人工作区。</p>
        {!isConfigured && !error ? <p className="mt-5 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">当前登录服务不可用，请稍后再试。</p> : null}
        ...
        {import.meta.env.DEV && authDebugInfo ? <div className="mt-3 flex gap-2 text-xs text-slate-500"><button type="button" onClick={handleCopyDebugInfo} className="rounded-full bg-white px-3 py-1.5 font-semibold ring-1 ring-slate-200">复制调试信息</button>{debugCopyStatus ? <span>{debugCopyStatus}</span> : null}</div> : null}
