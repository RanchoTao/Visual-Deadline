...
import { FilePlus2, ImagePlus, Link2, Mic, Square, X } from 'lucide-react';
...
export function CaptureComposer({ placeholder, submitLabel, labels, onSubmit }: { placeholder: string; submitLabel: string; labels: CaptureLabels; onSubmit: (draft: CaptureDraft) => void }) {
  ...
  return <form className="mt-10 w-full text-left sm:mt-12" onSubmit={(event) => { event.preventDefault(); if (hasDraft) onSubmit({ ...draft, text: draft.text.trim() }); }}>
    ...
    <div className="rounded-[1.6rem] border border-zinc-200/90 bg-white/90 p-3 shadow-[0_22px_70px_-46px_rgba(83,34,25,.34)] backdrop-blur-sm transition focus-within:border-zinc-400 sm:p-4">
      ...
      <div className="flex items-center justify-between gap-3 border-t border-zinc-100 px-1 pt-3"><div className="flex flex-wrap items-center gap-1"><input ref={imageInput} type="file" accept="image/*" multiple className="sr-only" onChange={(event) => { stageFiles(event.target.files); event.currentTarget.value = ''; }} /><input ref={fileInput} type="file" multiple className="sr-only" onChange={(event) => { stageFiles(event.target.files); event.currentTarget.value = ''; }} /><button type="button" title={labels.file} aria-label={labels.file} onClick={() => fileInput.current?.click()} className="inline-flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800"><FilePlus2 className="h-3.5 w-3.5" />{labels.file}</button><button type="button" title={labels.image} aria-label={labels.image} onClick={() => imageInput.current?.click()} className="inline-flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800"><ImagePlus className="h-3.5 w-3.5" />{labels.image}</button><button type="button" title={labels.url} aria-label={labels.url} onClick={() => setShowUrlInput((open) => !open)} className="inline-flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800"><Link2 className="h-3.5 w-3.5" />{labels.url}</button>{voiceSupported ? <button type="button" title={isRecording ? labels.stop : labels.voice} aria-label={isRecording ? labels.stop : labels.voice} onClick={isRecording ? stopVoiceRecording : () => void startVoiceRecording()} className={`inline-flex h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition-colors ${isRecording ? 'bg-red-50 text-red-700' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800'}`}>{isRecording ? <Square className="h-3.5 w-3.5" /> : <Mic className="h-3.5 w-3.5" />}{isRecording ? labels.stop : labels.voice}</button> : null}</div><button type="submit" aria-label={submitLabel} disabled={!hasDraft} className="flex h-10 min-w-10 items-center justify-center rounded-xl bg-[#d94a43] px-3 text-sm font-semibold text-white transition hover:bg-[#c43b35] disabled:cursor-not-allowed disabled:bg-zinc-200">{submitLabel}</button></div>
    </div>
  </form>;
}
