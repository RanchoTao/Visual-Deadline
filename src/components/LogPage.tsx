...
const tabs: { id: DataTab; label: string; description: string }[] = [
  ...
  { id: 'ai', label: 'AI洞察', description: '第三人称观察' },
];
...
export function LogPage({ tasks, goals, profile, pressure, pressureHistory, achievements, aiArtifacts, onAIReportGenerated, onDelete, onReviewNoteChange }: DataPageProps) {
  ...
  return <section className="space-y-6">
    ...
    {activeTab === 'health' ? <section className="space-y-6"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><StatCard label="身高" value={profile.height || '未记录'} detail="基础身体数据。" /><StatCard label="体重" value={profile.weight || '未记录'} detail="基础身体数据。" /><StatCard label="BMI" value={bmi ?? '等待'} detail={getHealthReadiness(profile)} /><StatCard label="恢复" value="待接入" detail="睡眠、HRV、静息心率、恢复分。" /></div><SectionShell eyebrow="健康整合" title="轻量健康系统"><p className="text-sm leading-7 text-slate-500">这里预留 Apple Health、KEEP、WHOOP、Garmin、Mi Fitness 等未来集成。VD 会把睡眠、运动、恢复、久坐、深夜活动与压力曲线放在同一个人类状态模型里。</p><div className="mt-5 grid gap-3 md:grid-cols-3"><StatCard label="运动" value="待接入" detail="跑量、训练时长、热量、VO₂ Max。" /><StatCard label="睡眠" value="待接入" detail="睡眠时长、一致性、深夜活动。" /><StatCard label="风险" value="观察中" detail="高压力与低恢复的组合风险。" /></div></SectionShell></section> : null}
    ...
    {activeTab === 'ai' ? <section className="space-y-6"><SectionShell eyebrow="AI洞察" title="第三人称行为观察"><p className="text-sm leading-7 text-slate-500">AI 不负责鼓励你。它负责观察：压力模式、行为倾向、恢复速度、长期任务稳定性、风险和节奏建议。</p><div className="mt-5 grid gap-3">{aiArtifacts.slice(0, 8).map((artifact) => <article key={artifact.id} className="rounded-3xl bg-white/75 p-4 ring-1 ring-white/80"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-slate-950">{artifact.title}</h3><time className="text-xs text-slate-400">{formatTime(artifact.createdAt)}</time></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-500">{artifact.content.slice(0, 360)}{artifact.content.length > 360 ? '…' : ''}</p></article>)}</div></SectionShell><AIReviewPanel tasks={tasks} pressureHistory={pressureHistory} onAIReportGenerated={onAIReportGenerated} /></section> : null}
  </section>;
}
