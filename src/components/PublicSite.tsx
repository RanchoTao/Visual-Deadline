...
function PublicFooter({ locale, onNavigate, onGetApp }: { locale: PublicLocale; onNavigate: (path: string) => void; onGetApp: () => void }) {
  ...
  const link = (path: string, label: string) => <a href={path} onClick={(event) => { event.preventDefault(); onNavigate(path); }} className="block text-left text-sm leading-7 text-zinc-500 hover:text-zinc-950">{label}</a>;
  return <footer className="relative mx-auto max-w-[1440px] px-5 pb-8 pt-20 sm:px-8 lg:px-12"><div className="grid gap-10 border-t border-zinc-200/80 pt-10 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.15fr_1fr]"><div><div className="flex items-center gap-2 text-sm font-semibold text-zinc-900"><img src="/logo.png" alt="" className="h-6 w-6 object-contain" />Visual Deadline</div><p className="mt-3 max-w-xs text-sm leading-6 text-zinc-500">{zh ? '把想法变成下一步。' : 'Turn thought into the next step.'}</p><div className="mt-4 flex gap-4 text-sm"><a href={branding.githubUrl} target="_blank" rel="noreferrer" className="text-zinc-500 hover:text-zinc-950">GitHub</a><a href="mailto:RanchoTao@gmail.com" className="text-zinc-500 hover:text-zinc-950">Email</a></div></div><div><h2 className="text-xs font-semibold tracking-[.14em] text-zinc-900">{zh ? '产品' : 'PRODUCT'}</h2><div className="mt-3 space-y-1">{link('/', zh ? 'Visual Deadline 网页版' : 'Visual Deadline web')}<button type="button" onClick={onGetApp} className="block text-left text-sm leading-7 text-zinc-500 hover:text-zinc-950">{zh ? 'Visual Deadline 移动端 · 即将推出' : 'Visual Deadline mobile · Coming Soon'}</button>{link('/docs', zh ? '文档' : 'Docs')}{link('/pricing', zh ? '定价' : 'Pricing')}</div></div><div><h2 className="text-xs font-semibold tracking-[.14em] text-zinc-900">{zh ? '研究' : 'RESEARCH'}</h2><div className="mt-3 space-y-1">{link('/research', zh ? '研究方向' : 'Research')}</div></div><div><h2 className="text-xs font-semibold tracking-[.14em] text-zinc-900">{zh ? '法务与安全' : 'LEGAL & SECURITY'}</h2><div className="mt-3 space-y-1">{link('/privacy', zh ? '隐私政策' : 'Privacy')}{link('/terms', zh ? '使用条款' : 'Terms')}{link('/security', zh ? '安全' : 'Security')}{link('/report-security', zh ? '报告安全问题' : 'Report security issue')}{link('/transparency', zh ? '透明度' : 'Transparency')}</div></div><div><h2 className="text-xs font-semibold tracking-[.14em] text-zinc-900">{zh ? '加入我们' : 'JOIN US'}</h2><div className="mt-3 space-y-1">{link('/careers', zh ? '加入我们' : 'Careers')}{link('/contact', zh ? '联系与合作' : 'Contact')}</div></div></div><div className="mt-12 border-t border-zinc-200/80 pt-5 text-xs text-zinc-400">© {new Date().getFullYear()} Visual Deadline</div></footer>;
}
