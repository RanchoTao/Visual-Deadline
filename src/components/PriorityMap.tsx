...
export function PriorityMap({ tasks }: PriorityMapProps) {
  ...
  function handlePointPointerDown(event: React.PointerEvent<HTMLButtonElement>, taskId: string) {
    if (event.pointerType !== 'mouse') {
      showPinnedDetail(taskId);
    }
  }

  function handlePointKeyDown(event: React.KeyboardEvent<HTMLButtonElement>, taskId: string) {
          ...
          {positionedTasks.map((positionedTask) => {
            ...
            return (
              <button
                key={task.id}
                type="button"
                onPointerDown={(event) => handlePointPointerDown(event, task.id)}
                onKeyDown={(event) => handlePointKeyDown(event, task.id)}
