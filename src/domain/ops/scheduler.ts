...
export function detectOpsConflicts(plan: OpsExecutionPlan, tasks: readonly Task[], state: OpsState): string[] {
  ...
  normalized.executors.forEach((executor) => {
    const lane = plan.allocations.filter((allocation) => allocation.executorId === executor.id && Date.parse(allocation.end) > Date.parse(allocation.start));
    const boundaries = [...new Set(lane.flatMap((allocation) => [Date.parse(allocation.start), Date.parse(allocation.end)]))].sort((left, right) => left - right);
    for (let index = 0; index < boundaries.length - 1; index += 1) {
      const start = boundaries[index]; const activeAtInterval = lane.filter((allocation) => Date.parse(allocation.start) <= start && Date.parse(allocation.end) > start);
      if (activeAtInterval.length > executor.maxParallel) conflicts.push(`${activeAtInterval.every((allocation) => allocation.locked) ? 'LOCKED_CAPACITY_CONFLICT' : 'CAPACITY_CONFLICT'}:${executor.id}`);
    }
  });
