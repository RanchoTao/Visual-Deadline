...
export function buildCaptureMaterializationPlan(interpretation: CaptureInterpretation, existingTasks: Task[], existingGoals: Goal[], _now = new Date()): CaptureMaterializationPlan {
  ...
  const goals = interpretation.goals.filter((draft) => draft.included).flatMap((draft) => { if (goalTitles.has(draft.title.toLocaleLowerCase())) { duplicateWarnings.push(`已存在相似目标：${draft.title}`); return []; } return [{ draftId: draft.id, input: { title: draft.title, targetDate: draft.targetDate, category: draft.category, priority: draft.priority, linkedTaskIds: [] } }]; });
