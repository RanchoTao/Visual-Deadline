...
function normalizeEvent(value: unknown): ReviewHistoryEvent | undefined {
  ...
  const isPressureEvent = kind === 'pressure_sample' || kind === 'pressure_recalibrated';
  const pressureSource = isPressureEvent ? source.pressureSource === 'manual' || source.pressureSource === 'task_derived' ? source.pressureSource : 'unknown' : source.pressureSource === 'manual' || source.pressureSource === 'task_derived' || source.pressureSource === 'unknown' ? source.pressureSource : undefined;
  const persistedEvidenceSource = source.evidenceSource === 'captured_live' || source.evidenceSource === 'legacy_backfill' || source.evidenceSource === 'derived' ? source.evidenceSource : undefined;
  const evidenceSource: ReviewEvidenceSource = isPressureEvent ? pressureSource === 'task_derived' ? 'derived' : pressureSource === 'manual' ? 'captured_live' : 'legacy_backfill' : persistedEvidenceSource ?? (kind === 'review_saved' || kind === 'ai_review_generated' ? 'captured_live' : 'legacy_backfill');
