...
export function deriveReviewMetrics(input: { tasks: readonly Task[]; goals: readonly Goal[]; pressureHistory: readonly PressureHistoryRecord[]; opsState: OpsState; window: { start: string; end: string }; now: string }): ReviewMetricsSnapshot {
  const completed = input.tasks.filter((task) => task.lifecycleStatus === 'completed' && isInsideReviewWindow(task.completedAt, input.window));
  const abandoned = input.tasks.filter((task) => task.lifecycleStatus === 'abandoned' && isInsideReviewWindow(task.abandonedAt, input.window));
  const deadlineTasks = completed.filter((task) => valid(task.deadline) !== undefined && valid(task.completedAt) !== undefined);
  const deltas = deadlineTasks.map((task) => (Date.parse(task.deadline!) - Date.parse(task.completedAt!)) / hour);
  const pressureSamples = input.pressureHistory.filter((sample) => isInsideReviewWindow(sample.timestamp, input.window) && Number.isFinite(sample.pressure));
