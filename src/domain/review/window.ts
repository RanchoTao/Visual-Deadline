...
export function reviewDateKey(timestamp: string, timezone: string): string | undefined {
  const instant = Date.parse(timestamp); if (!Number.isFinite(instant)) return undefined; const parts = zonedParts(instant, validTimezone(timezone)); return dateKey(parts.year, parts.month, parts.day);
}

export function createReviewWindow(days: ReviewWindowDays, now: string, timezone = 'UTC'): ReviewWindow {
  const parsedEnd = Date.parse(now); const end = Number.isFinite(parsedEnd) ? parsedEnd : Date.now(); const safeTimezone = validTimezone(timezone); const localEnd = zonedParts(end, safeTimezone);
